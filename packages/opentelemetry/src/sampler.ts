...
export class SentrySampler implements Sampler {
  ...
  public shouldSample(
    ...
    // `ATTR_HTTP_REQUEST_METHOD` is the new attribute, but we still support the old one, `ATTR_HTTP_METHOD`, for now.
    // eslint-disable-next-line deprecation/deprecation
    const maybeSpanHttpMethod = spanAttributes[ATTR_HTTP_METHOD] || spanAttributes[ATTR_HTTP_REQUEST_METHOD];
...
function getBaseTraceState(context: Context, spanAttributes: SpanAttributes): TraceStateInterface {
  ...
  // eslint-disable-next-line deprecation/deprecation
  const url = spanAttributes[ATTR_HTTP_URL] || spanAttributes[ATTR_URL_FULL];

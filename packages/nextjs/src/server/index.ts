...
export function init(options: NodeOptions): NodeClient | undefined {
  ...
  client?.on('beforeSampling', ({ spanAttributes, spanName, parentSampled, parentContext }, samplingDecision) => {
    ...
    if (
      ...
      typeof spanAttributes[ATTR_HTTP_TARGET] === 'string' &&
      // eslint-disable-next-line deprecation/deprecation
      spanAttributes[ATTR_HTTP_TARGET].includes('sentry_key') &&
      // eslint-disable-next-line deprecation/deprecation
      spanAttributes[ATTR_HTTP_TARGET].includes('sentry_client')
    ) {
      samplingDecision.decision = false;
    }
  });
  ...
  client?.on('spanStart', span => {
    ...
    if (spanAttributes?.['next.route']) {
      ...
      if (
        // eslint-disable-next-line deprecation/deprecation
        (rootSpanAttributes?.[ATTR_HTTP_REQUEST_METHOD] || rootSpanAttributes?.[ATTR_HTTP_METHOD]) &&
